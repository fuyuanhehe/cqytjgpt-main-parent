package com.ccttic.cqytjgpt.enterpriseapi.controller.role;

import com.ccttic.cqytjgpt.enterpriseapi.auth.UrlFilterInvocationSecurityMetadataSource;
...
public class RoleController {
 ...
 private	UrlFilterInvocationSecurityMetadataSource soruce;
	...
	@ApiImplicitParams({
	    ...
	    @ApiImplicitParam(name="list",value="角色id列表(list<String>)",required=true,paramType="form"),
	})
	 ...
	 * @date  2018年6月1日
	 */
	@OperLogging(operType = 3,content="查询角色列表")
	@RequestMapping(value="/RolePages",method={RequestMethod.POST})
	...
	@ApiOperation(value="查询角色列表",notes="access_token，必传值")
	@ApiImplicitParams({
			@ApiImplicitParam(name="access_token",value="access_token",required=true,paramType="query"),
			@ApiImplicitParam(name="roleNm",value="角色名称",required=false,paramType="form"),
			@ApiImplicitParam(name="roleCd",value="角色编码",required=false,paramType="form"),
	...
	public ResponseMsg<List<Roles>> rolePages(@RequestBody(required = false) rolesPage tment)  {
		ResponseMsg<List<Roles>> resp = new ResponseMsg<List<Roles>>();
		try {
			PageRequest page = new PageRequest();
			page.setPage(tment.getPage());
			page.setRows(tment.getRows());
			Page<Roles> pager = this.roleService.listRole(page,tment);
